  ...
  componentWillMount(){
    this._panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (e,gs)=>{
        console.log(this.state.is_1s_later);
        var temp = {
          x:gs.dx,
          y:gs.dy,
        }
      ...
      onPanResponderMove: (e,gs)=>{
        if(this.state.can_push){
            if(gs.dy<-150){
              ...
              clearTimeout(this.long_start_opacity);
              clearInterval(this.vibrate_controller);
              clearInterval(this.emergency);
              emergency_opacity = 0;
              this.setState({
                emergency_opacity:emergency_opacity,
                is_3s_later:false,
                is_1s_later:false,
                can_push:false
              });
