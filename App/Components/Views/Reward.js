    ...
    return Global.eventArr.map(function(news, i){
      ...
      return(
        <TouchableOpacity onPress={()=>{Actions.eventdetail(
          ...
          {image}
          <View style={{height:230,width:width,position:'absolute',top:0,left:0,alignItems:'center',justifyContent:'center'}}>
            <Text style={{fontSize:30,color:'white'}}>{news.title}</Text>
            <View style={{backgroundColor:'rgba(20,139,205,0.8)',borderRadius:4,width:250,height:50,alignItems:'center',justifyContent:'center'}}><Text style={{fontSize:24,color:'white'}}>{news.date}</Text></View>
          </View>
        </TouchableOpacity>
      )
    });
